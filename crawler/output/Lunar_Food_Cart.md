<!-- https://growtopiawiki.com/w/Lunar_Food_Cart -->

# Lunar Food Cart

Serve out some festive dishes with your own food cart!
Properties
This item never drops any seeds.
This item can be placed in two directions, depending on the direction you're facing.
Data: Paint PreviewType — Platform – Foreground — Aqua Blue Charcoal Green Purple Red Varnish Yellow
Render Type: Single
Collision Type: No Collision
Hardness: **5 Hits** **4 Hits** – Restores after **4s** of inactivity
Gems Drop: **N/A**
**1h**
Paint Preview Aqua Blue Charcoal Green Purple Red Varnish Yellow
During Lunar New Year

|
Consumption

Consume a(n)  Lunar Chest **(x1)**.
May yield **1** item each.

 |
The **Lunar Food Cart** is an unsplicable platform block which was added as part of Lunar New Year 2024 on February 1, 2024.
##  Function
A Lunar Food Cart behaves as a platform thus a player can jump pass through it from underneath.
